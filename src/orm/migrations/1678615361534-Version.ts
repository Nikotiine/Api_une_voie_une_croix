...
export class Version1678615361534 implements MigrationInterface {
    name = 'Version1678615361534'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`exposition\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_50aaaf23ba3e63c19c565451b4\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`engagement\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_336dad01ab7e3ea1a0c0e2ed69\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`equipment\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_99935ab824cba9096f178988dc\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`level\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_b1644a53747694faf5869b0394\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`region\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`department\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`lat\` double NOT NULL, \`lng\` double NOT NULL, \`regionId\` bigint NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`route_profile\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_f7619fc5ea6e0cad7b57140bfd\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`rock_type\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_8ebddfc1bc8400247a43785176\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`secteur\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`siteId\` bigint NULL, UNIQUE INDEX \`IDX_63f3b02799a16a09851e6eee8e\` (\`name\`, \`siteId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`approach_type\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`label\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_3bbe18bbcf18cf9ccb9a02eba6\` (\`label\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`firstName\` varchar(255) NOT NULL, \`lastName\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`birthday\` datetime NOT NULL, \`createdAt\` datetime NOT NULL, \`updatedAt\` datetime NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
